...
import wave
import threading
import keyboard  # Изменено с pynput на keyboard
import pyautogui
import pyperclip
import io
import base64
...
class AudioTranscriber:
    ...
    def start_listening(self):
        ...
        # Проиграть звук "ding" при старте (синхронно)
    ...
    def stop_listening(self):
        ...
        if self.is_recording:
            ...
            # Проиграть звук "dong" при окончании (синхронно)
...
- Тематика: IT, программирование, системное администрирование
- Часто встречаются английские технические термины (server, backup, snapmirror, destination, docker, kubernetes и т.д.)

ЗАДАЧА: Выбрать ОДИН из двух вариантов (RU или EN) целиком.

ЗАПРЕЩЕНО:
- Склеивать слова из RU и EN вместе (например "действительно destination" - НЕПРАВИЛЬНО!)
- Комбинировать два языка в одной фразе (кроме случая смешанной речи ниже)
- Придумывать новые слова

РАЗРЕШЕНО комбинировать ТОЛЬКО если:
- В RU есть явная транслитерация английского IT-термина ("снап мир", "докер")
- И в EN есть правильный английский термин ("snapmirror", "docker")
- Тогда замени транслитерацию на английский термин

Во всех остальных случаях: выбери ОДИН вариант целиком (RU или EN).

Алгоритм:
1. Проверь: есть ли в RU транслитерация английских IT-терминов?
   - "снап мир", "докер", "сервер" (транслитерация) - ДА
   - "действительно", "хранилище", "проверка" (нормальные русские слова) - НЕТ
   
2. Если НЕТ транслитерации:
   - Выбери ОДИН вариант целиком
   - Приоритет: RU (если осмысленный) > EN (если RU бессмысленный)
   - Примеры:
     * RU: "действительно" + EN: "destination" → "действительно" (выбрали RU целиком)
     * RU: "хранилище" + EN: "storage" → "хранилище" (выбрали RU целиком)
   
3. Если ЕСТЬ транслитерация:
   - Возьми RU как основу
   - Замени транслитерацию на английский термин из EN
   - Пример: RU: "репликация снап мир" + EN: "snapmirror" → "репликация snapmirror"

Примеры:
- RU: "целевой хранилище для репликации над миру и же" + EN: "snap mirror destination" → "целевое хранилище для репликации snapmirror destination"
- RU: "запустить докер контейнер" + EN: "docker container" → "запустить docker container"
...
def main():
    ...
    # Регистрация горячих клавиш через keyboard (более надежно)
    try:
        keyboard.add_hotkey('alt+`', on_activate_record)
